...
namespace Kafka.Streams.Tests.Kstream.Internals
{
    public class KStreamWindowAggregateTest
    {
        ...
        public void testAggBasic()
        {
            ...
            IKTable<IWindowed<string>, string> table2 = builder
                ...
                .Aggregate(MockInitializer.STRING_INIT, MockAggregator.TOSTRING_ADDER, Materialized.As < string, string, IWindowStore<Bytes, byte[]>("topic1-Canonized").withValueSerde(Serdes.String()));
        ...
        public void testJoin()
        {
            ...
            IKTable<IWindowed<string>, string> table2 = builder
                ...
                .GroupByKey(Grouped.With(Serdes.String(), Serdes.String()))
                .WindowedBy(TimeWindows.of(TimeSpan.FromMilliseconds(10)).advanceBy(TimeSpan.FromMilliseconds(5)))
                .Aggregate(MockInitializer.STRING_INIT, MockAggregator.TOSTRING_ADDER, Materialized.As < string, string, IWindowStore<Bytes, byte[]>("topic2-Canonized").withValueSerde(Serdes.String()));
            table2.ToStream().Process(supplier);
            ...
            processors.First().CheckAndClearProcessResult(
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(0, 10)), "0+1+1+1", 9),
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(5, 15)), "0+1+1", 9),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(0, 10)), "0+2+2", 6),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(5, 15)), "0+2", 6),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(0, 10)), "0+4+4", 7),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(5, 15)), "0+4", 7),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(0, 10)), "0+2+2+2", 8),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(5, 15)), "0+2+2", 8),
                new KeyValueTimestamp<>(new Windowed<string>("C", new TimeWindow(0, 10)), "0+3+3", 9),
                new KeyValueTimestamp<>(new Windowed<string>("C", new TimeWindow(5, 15)), "0+3", 9)
            );
            processors.Get(1).checkAndClearProcessResult(System.Array.Empty<Streams.KeyValueTimestamp>());
            processors.Get(2).checkAndClearProcessResult(System.Array.Empty<Streams.KeyValueTimestamp>());
            ...
            processors.Get(0).checkAndClearProcessResult(System.Array.Empty<Streams.KeyValueTimestamp>());
            processors.Get(1).checkAndClearProcessResult(
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(0, 10)), "0+a", 0),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(0, 10)), "0+b", 1),
                new KeyValueTimestamp<>(new Windowed<string>("C", new TimeWindow(0, 10)), "0+c", 2),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(15, 25)), "0+d", 20),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(20, 30)), "0+d", 20),
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(15, 25)), "0+a", 20),
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(20, 30)), "0+a", 20)
            );
            processors.Get(2).checkAndClearProcessResult(
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(0, 10)), "0+1+1+1%0+a", 9),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(0, 10)), "0+2+2+2%0+b", 8),
                new KeyValueTimestamp<>(new Windowed<string>("C", new TimeWindow(0, 10)), "0+3+3%0+c", 9));
            ...
            processors.Get(0).checkAndClearProcessResult(System.Array.Empty<Streams.KeyValueTimestamp>());
            processors.Get(1).checkAndClearProcessResult(
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(0, 10)), "0+a+a", 5),
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(5, 15)), "0+a", 5),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(0, 10)), "0+b+b", 6),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(5, 15)), "0+b", 6),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(0, 10)), "0+d", 7),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(5, 15)), "0+d", 7),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(10, 20)), "0+d", 18),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(15, 25)), "0+d+d", 20),
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(15, 25)), "0+a+a", 21),
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(20, 30)), "0+a+a", 21)
            );
            processors.Get(2).checkAndClearProcessResult(
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(0, 10)), "0+1+1+1%0+a+a", 9),
                new KeyValueTimestamp<>(new Windowed<string>("A", new TimeWindow(5, 15)), "0+1+1%0+a", 9),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(0, 10)), "0+2+2+2%0+b+b", 8),
                new KeyValueTimestamp<>(new Windowed<string>("B", new TimeWindow(5, 15)), "0+2+2%0+b", 8),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(0, 10)), "0+4+4%0+d", 7),
                new KeyValueTimestamp<>(new Windowed<string>("D", new TimeWindow(5, 15)), "0+4%0+d", 7)
            );
        }
    }
    ...
    public void shouldLogAndMeterWhenSkippingNullKey()
    {
        ...
        builder
            ...
            .Aggregate(
                ...
                Materialized.As < string, string, IWindowStore<Bytes, byte[]>("topic1-Canonicalized").withValueSerde(Serdes.String())
            );
    ...
    public void shouldLogAndMeterWhenSkippingExpiredWindow()
    {
        ...
        IKStream<K, V> stream1 = builder.Stream(topic, Consumed.With(Serdes.String(), Serdes.String()));
