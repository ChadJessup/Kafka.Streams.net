...
{
    public class TaskCreator<K, V> : AbstractTaskCreator<StreamTask<K, V>>
    {
        ...
        StreamTask<K, V> createTask(IConsumer<byte[], byte[]> consumer,
                              TaskId taskId,
                              HashSet<TopicPartition> partitions)
        {
            createTaskSensor.record();

            return new StreamTask<K, V>(
                taskId,
                partitions,
                                          ...
                                          int threadIdx)
        {
            ...
            AbstractTaskCreator<StreamTask<K, V>> activeTaskCreator = new TaskCreator(
                builder,
                config,
                streamsMetrics,
                stateDirectory,
        ...
        private void addRecordsToTasks(ConsumerRecords<byte[], byte[]> records)
        {
            ...
            {
                StreamTask<K, V> task = taskManager.activeTask(partition);
        ...
        private void updateThreadMetadata(Dictionary<TaskId, StreamTask<K, V>> activeTasks,
                                          Dictionary<TaskId, StandbyTask> standbyTasks)
        {
            ...
            foreach (KeyValuePair<TaskId, StreamTask<K, V>> task in activeTasks)
            {
        ...
        public Dictionary<TaskId, StreamTask<K, V>> tasks()
        {
        ...
        public Dictionary<MetricName, IMetric> producerMetrics()
        {
            ...
            else
            {
                ...
                foreach (StreamTask<K, V> task in taskManager.activeTasks().Values)
                {
