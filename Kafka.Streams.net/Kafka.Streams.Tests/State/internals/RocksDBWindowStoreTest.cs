...
public class RocksDBWindowStoreTest : WindowBytesStoreTest {
    ...
    public void ShouldOnlyIterateOpenSegments() {
        ...
        WindowStoreIterator<string> iterator = windowStore.fetch(1, 0L, currentTime);
        ...
        Assert.Equal(new KeyValuePair<>(SEGMENT_INTERVAL, "two"), iterator.next());
        Assert.Equal(new KeyValuePair<>(2 * SEGMENT_INTERVAL, "three"), iterator.next());
    ...
    public void TestInitialLoading() {
        ...
        try { 
 (WindowStoreIterator iter = windowStore.fetch(0, ofEpochMilli(0L), ofEpochMilli(1000000L)));
            while (iter.hasNext()) {
                iter.next();
            }
        }
